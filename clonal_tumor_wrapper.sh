if [ -z "$2" ]
then
        echo 'Usage: sh clonal_heterogeneity_wrapper.sh <selector> <window_size>'
        exit 1
fi
selector=$1
window_size=$2
...
parallel --jobs 8 --xapply sh filterMutatedRows.sh bamfiles.txt :::: normfiles.txt :::: freqfiles.txt :::: cosmicfiles.txt ::: $selector ::: $window_size
