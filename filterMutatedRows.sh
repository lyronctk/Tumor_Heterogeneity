...
echo '----Summarizing cosmic file'
cut -f13-14 $cosmic_file | tail -n +2 > $file_name.summarized.cosmic.split.txt

clang++ -std=c++11 -stdlib=libc++ generateWindows.cpp
echo "$file_name.mutatedrows.txt $selector $germline_file $freq_file $file_name.refremoved.txt $cosmic_file $window_length $window_slide $file_name.windowcounts.txt" | ./a.out